    ...
    socket.emit("admin_list_messages_by_user", params , messages=>{
        ...
        messages.forEach(message=>{
            ...
            if(message.admin_id===null){
                ...
                createDiv.innerHTML = "<span>"+connection.user.email+  "<br><br>" + message.text + "</span>"
                createDiv.innerHTML += `<span class="admin_date">${dayjs(message.created_at).format("DD/MM/YYYY HH:mm:ss")}</span> `
            }else{
                createDiv.className = "admin_message_admin";

                createDiv.innerHTML = `Atendente: <span style="color:black;"><br>${message.text}</span>`;
                createDiv.innerHTML += `<span class="admin_date">${dayjs(message.created_at).format("DD/MM/YYYY HH:mm:ss")}</span> `
            }
...
function sendMessage(id){
    ...
    createDiv.innerHTML = `Atendente: <span style="color:black;"><br>${params.text}</span>`;
...
socket.on("admin_receive_message", data => {
    ...
    createDiv.innerHTML = "<span>"+connection.user.email+  "<br><br>" + data.message.text + "</span>"
